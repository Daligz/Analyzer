...
namespace Analyzer.Tokens
{
    public static class Expressions
    {
        ...
        public static Regex whiteSpacesDefinition = new Regex(@"^\s+|\s+$|\s+(?=\s)");

        public static List<Regex> GetExpressionByToken(TokenType tokenType)
        {
            switch (tokenType)
            {
                case TokenType.Identifiers:
                    return new List<Regex> { variableDefinition, operationsDefinition };
                case TokenType.Operators:
                    return new List<Regex> { operatorsDefinition };
                case TokenType.ConstStrings:
                    return new List<Regex> { stringDefinition };
                case TokenType.ReservedWords:
                    return new List<Regex> { reservedDefinition };
                case TokenType.ConstNums:
                    return new List<Regex> { constNumeric };
                case TokenType.Special:
                    return new List<Regex> { specialDefinition };
                default:
                    return null;
            }
        }
